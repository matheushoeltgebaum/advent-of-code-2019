        ...
        switch (direction) {
            case 'U':
                for (let i = 1; i <= count; i++) {
                    y += 1;
                    combinedSteps = totalCombinedSteps + Math.abs(y);
                    ...
                    if (indexIntersection > -1) {
                        ...
                        if (wireNumber === 2 && positions[indexIntersection].countStepsSecondWire === 0) {
                            positions[indexIntersection].countStepsSecondWire = combinedSteps;
                        }
                    } else {
                        positions.push({ x: x, y: y, wireNumber: wireNumber, hasIntersection: false, countStepsFirstWire: combinedSteps, countStepsSecondWire: 0 });
                    }
                }
                break;
            case 'L':
                for (let i = 1; i <= count; i++) {
                    x -= 1;
                    combinedSteps = totalCombinedSteps + Math.abs(x);
                    indexIntersection = positions.findIndex(p => p.x === x && p.y === y && p.wireNumber !== wireNumber);
                    if (indexIntersection > -1) {
                        positions[indexIntersection].hasIntersection = true;
                        if (wireNumber === 2 && positions[indexIntersection].countStepsSecondWire === 0) {
...
const calculateMinDistance = function(coordinates) {
    ...
    for (let coordinate of coordinates) {
        ...
        currentDistance = Math.abs(0 - x) + Math.abs(0 - y);
