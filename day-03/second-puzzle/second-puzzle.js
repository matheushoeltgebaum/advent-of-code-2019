...
const populateMatrix = function (positions, coordinates, wireNumber) {
    let x = 0;
    let y = 0;
    let totalCombinedSteps = 0;

    for (let coordinate of coordinates) {
        let direction = coordinate[0];
        let count = Number(coordinate.replace(direction, ''));
        let indexIntersection = -1;

        switch (direction) {
            case 'U':
                for (let i = 1; i <= count; i++) {
                    y += 1;
                    indexIntersection = positions.findIndex(p => p.x === x && p.y === y && p.wireNumber !== wireNumber);
                    if (indexIntersection > -1) {
                        positions[indexIntersection].hasIntersection = true;
                        if (wireNumber === 2 && positions[indexIntersection].countStepsSecondWire === 0) {
                            positions[indexIntersection].countStepsSecondWire = ++totalCombinedSteps;
                        }
                    } else {
                        positions.push({ x: x, y: y, wireNumber: wireNumber, hasIntersection: false, countStepsFirstWire: ++totalCombinedSteps, countStepsSecondWire: 0 });
                    }
                }
                break;
            case 'L':
...
const calculateMinDistance = function(coordinates) {
    let minDistance = Number.MAX_SAFE_INTEGER;
    let currentDistance = 0;
    let countFirstWire = 0;
    let countSecondWire = 0;

    for (let coordinate of coordinates) {
        countFirstWire = coordinate.countStepsFirstWire;
        countSecondWire = coordinate.countStepsSecondWire;
        currentDistance = countFirstWire + countSecondWire;
...
positions.push({ x: 0, y: 0, wireNumber: 0, hasIntersection: false });
positions = populateMatrix(positions, firstWire, 1);
positions = populateMatrix(positions, secondWire, 2);

let minDistance = calculateMinDistance(positions.filter(p => p.hasIntersection));
